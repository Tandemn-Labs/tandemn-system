...
def plot_timeseries(
    ...
    try:
        ...
        raise RuntimeError("matplotlib and numpy are required for plotting. Install with: pip install matplotlib numpy")
    ...
    if "throughput" in ax_map:
        ...
        if np.any(valid_gen):
            ...
            if len(active) > 0:
                ...
                ax.axhline(mean_gen, color=COLORS["gen_throughput"], linestyle="--", alpha=0.5, linewidth=1.0)
                ax.text(
                    t_plot[-1],
                    mean_gen,
                    f"  avg {mean_gen:.0f}",
        ...
        if np.any(valid_prompt):
            ax.scatter(
                t_plot[valid_prompt],
                prompt_tp[valid_prompt],
                color=COLORS["prompt_throughput"],
                alpha=0.40,
                s=15,
                label="Prompt tok/s",
                zorder=2,
    ...
    if "kv_cache" in ax_map:
        ...
        if np.any(valid):
            ax.plot(t_plot[valid], kv_pct[valid], color=COLORS["kv_cache"], linewidth=1.5, label="KV Cache Util.")
            ax.fill_between(t_plot[valid], 0, kv_pct[valid], alpha=0.3, color=COLORS["kv_cache"])
    ...
    if "scheduler" in ax_map:
        ...
        for arr, name, color_key in [
            ...
            valid = ~np.isnan(arr)
            if np.any(valid):
                ax.plot(t_plot[valid], arr[valid], color=COLORS[color_key], linewidth=1.5, label=name)
    ...
    if "ttft" in ax_map:
        ...
        for arr, style, label in [
            ...
            valid = ~np.isnan(arr)
            if np.any(valid):
                ax.plot(t_plot[valid], arr[valid], color=base_color, linestyle=style, linewidth=1.5, label=label)
    ...
    if "completions" in ax_map:
        ...
        if np.any(valid_c):
            ax.plot(
                t_plot[valid_c], completions[valid_c], color=COLORS["completions"], linewidth=1.5, label="Completions"
            )
        if np.any(valid_p):
            ax.plot(
                t_plot[valid_p], preemptions[valid_p], color=COLORS["preemptions"], linewidth=1.5, label="Preemptions"
            )
    ...
    if "summary" in ax_map and summary:
        ...
        for col_idx, (section_title, items) in enumerate(sections):
            cx = col_x[col_idx]
            ax.text(cx, y_top, section_title, fontsize=8, fontweight="bold", color="#1a1a1a", transform=ax.transAxes)
            for i, (label, value) in enumerate(items):
                y = y_top - (i + 1) * line_h
                ax.text(cx + 0.01, y, f"{label}:", fontsize=7, color="#555555", transform=ax.transAxes)
                ax.text(cx + 0.13, y, value, fontsize=7, fontweight="bold", color="#1a1a1a", transform=ax.transAxes)
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Generate publication-quality PDF from Orca timeseries.csv")
    parser.add_argument("csv_path", type=str, help="Path to timeseries.csv")
    parser.add_argument(
        "--output", "-o", type=str, default=None, help="Output PDF path (default: timeseries.pdf next to CSV)"
    )
    parser.add_argument("--metrics", "-m", type=str, default=None, help="Path to metrics.csv for summary panel")
