...
class BatchedRequest(BaseModel):
    ...
    num_lines: Optional[int] = (
        None  # number of prompt lines in the file (parsed from input_file)
    )
    ...
    avg_output_tokens: Optional[int]  # expected avg output per line
    ...
    def _validate_user_specified(self):
        if self.placement_solver == "user_specified":
            if self.gpu_type is None:
                raise ValueError(
                    "gpu_type is required when placement_solver='user_specified'"
                )
            if self.tp_size is not None and self.tp_size not in {1, 2, 4, 8}:
                raise ValueError(
                    f"tp_size must be one of {{1, 2, 4, 8}}, got {self.tp_size}"
                )
            if self.pp_size is not None and self.pp_size not in {1, 2, 3, 4}:
                raise ValueError(
                    f"pp_size must be one of {{1, 2, 3, 4}}, got {self.pp_size}"
                )
        return self
...
class vLLMSpecificConfig(BaseModel):
    ...
    config_format: Optional[Literal["auto", "mistral"]] = (
        "auto"  # use mistral if mistral model is detected
    )
    # persona 2 config
    tokenizer: Optional[str] = (
        None  # leave this field blank if not specified (just the path of the tokenizer)
    )
    tokenizer_mode: Optional[Literal["auto", "deepseek", "mistral"]] = (
        "auto"  # Leave this to Auto if not specified
    )
    kv_cache_dtype: Optional[
        Literal[
            "auto", "bfloat16", "fp8", "fp8_ds_mla", "fp8_e4m3", "fp8_e5m2", "fp8_inc"
        ]
    ] = None
