...
class VPCQuotaTracker:
    ...
    def reserve_cluster(
        self, cluster_name: str, region: str, market: str, instance_type: str, num_instances: int
    ) -> bool:
        ...
        with self.lock:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    (cluster_name, region, market, instance_type, num_instances, family_type, vcpu, time.time()),
                )
            # In-memory update only after successful DB write
            self.used_vcpu[(region, market, family_type)] = self.used_vcpu.get((region, market, family_type), 0) + vcpu

        logger.info(
            f"[QuotaTracker] Reserved cluster {cluster_name}: {instance_type} x{num_instances} in {region}/{market}"
        )
        return True
...
class JobState:
    ...
    gpu_base: str | None = None  # added
    tp: int | None = None
    pp: int | None = None
    replicas: int | None = None
    allocated_gpus: int | None = None  # added
    vcpu_needed: int | None = None  # #added
    instance_types: str | None = None  # added
    num_instances: int | None = None  # added
    instance_ids: list[str] | None = None  # added
    allocations: list[tuple[str, int]] | None = None  # added
...
class JobRecord:
    ...
    output_s3_path: str | None = None  # S3 path for output file download
